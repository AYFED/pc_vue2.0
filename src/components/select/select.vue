    ...
    <div
        ...
        <transition name="transition-drop">
            <Drop
                ...
                <ul v-show="loading" :class="[prefixCls + '-loading']"><i class="ay-load-loop ay-icon ay-icon-ios-sync"></i>{{ localeLoadingText }}</ul>
            </Drop>
        </transition>
    </div>
</template>
<script>
  ...
  const optionRegexp = /^i-option$|^Option$|^XOption$/i
  ...
  export default {
      ...
      selectOptions () {
        ...
        if (this.autoComplete) {
          ...
          return slotOptions.map(node => {
            if (node === selectedSlotOption || getNestedProperty(node, 'componentOptions.propsData.value') ===
              this.value) return applyProp(node, 'isFocused', true)
    ...
    methods: {
      ...
      getInitialValue () {
        ...
        if (!multiple && (typeof initialValue[0] === 'undefined' ||
          (String(initialValue[0]).trim() === '' && !Number.isFinite(initialValue[0])))) initialValue = []
    ...
    watch: {
      ...
      focusIndex (index) {
        ...
        let bottomOverflowDistance = optionInstance.$el.getBoundingClientRect().bottom -
          this.$refs.dropdown.$el.getBoundingClientRect().bottom
        let topOverflowDistance = optionInstance.$el.getBoundingClientRect().top -
